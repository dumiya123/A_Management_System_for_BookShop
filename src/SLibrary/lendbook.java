   ...
   ResultSet rs;
    ...
    public void Connect()
    {
        ...
        try 
        {
            ...
            Logger.getLogger(lendbook.class.getName()).log(Level.INFO, "Database connection established.");
        } catch (SQLException | ClassNotFoundException ex) {
             Logger.getLogger(lendbook.class.getName()).log(Level.SEVERE, "Database connection failed.", ex);
...
//    }
    ...
    public void Author()
    {
        try {
            ...
            while(rs.next())
            {
                txtbook.addItem(new AuthorItem(rs.getInt(1),rs.getString(2)));
            }
        } catch (SQLException ex) {
            Logger.getLogger(lendbook.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//                   "JOIN author a ON b.author = a.`Category ID` " +
//                   "JOIN publisher p ON b.publisher = p.`Category ID`");
          pst = conn.prepareStatement("SELECT l.memberid, m.name, b.bname, l.issuedate, l.returndate " +
                                      "FROM lendbook l " +
                        ...
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(jPanel1Layout.createSequentialGroup()
                                ...
                                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                    ...
                                    .addComponent(txtissuedate, javax.swing.GroupLayout.DEFAULT_SIZE, 254, Short.MAX_VALUE)
    ...
    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
       ...
       // txtbook.setText(d1.getValueAt(selectIndex, 2).toString());
        ...
        // Get the date values from your TableModel
        String issueDateStr = d1.getValueAt(selectIndex, 3).toString();
        String returnDateStr = d1.getValueAt(selectIndex, 4).toString();

        // Create SimpleDateFormat for parsing date strings
       ...
       try {
             ...
             // Set the dates in the JDateChooser fields
    ...
    private void txtidKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtidKeyPressed
        ...
        if(evt.getKeyCode()== KeyEvent.VK_ENTER)
        {
            String mid=txtid.getText();
            try {
                pst = conn.prepareStatement("SELECT * FROM member WHERE `Category ID` = ?");
...
//        String id_one=txtid.getText();
        String member=txtmember.getText();
        Date issuedate=txtissuedate.getDate();
        Date returndate=txtrdate.getDate();
    ...
    if (issuedate == null || returndate == null) {
        ...
        return; // Exit the method early
           ...
           // pst=conn.prepareStatement("INSERT INTO 'category'('Category_name','Status')values(?,?)");
            //pst = conn.prepareStatement("INSERT INTO `Category` (`Category_name`, `status`) VALUES (?, ?)");
            //pst = conn.prepareStatement("update category set Category_Name= ?, Status= ? where Category ID= ?");
            pst = conn.prepareStatement("UPDATE lendbook SET  memberid = ?, bookid = ?, issuedate = ?, returndate = ? WHERE `id` = ?");
...
//            pst.setString(1, id_one);
           // pst.setString(2,address );
            ...
            pst.setString(2, txtbook.getSelectedItem().toString());
...
//            
            ...
            int rowsAffected=pst.executeUpdate();
               ...
               if(rowsAffected>0)
               {
                   JOptionPane.showMessageDialog(this, "Lending Book Table updated");
                    txtid.setText("");
                    txtmember.setText("");
                    txtissuedate.setDate(null); // Clear the date
                    txtrdate.setDate(null); // Clear the date
                    txtid.requestFocus();
                    jButton2.setEnabled(true);
               }
               else
               {
                   JOptionPane.showMessageDialog(this, "Error updating Lending Book Table.");
               }
